#ifndef PICO_RP2040_USB_HID_H
#define PICO_RP2040_USB_HID_H
...
void tud_hid_set_report_cb(uint8_t instance, uint8_t report_id, hid_report_type_t report_type, uint8_t const *buffer, uint16_t bufsize) {
    ...
    memcpy(&packet_rx, buffer, SIZE_PACKET_RX_T);
    packet_rx.args_size = bufsize - sizeof(packet_rx.id) - sizeof(packet_rx.cmd);

    packet_tx_t packet_tx = dispatch(packet_rx);

    if (tud_hid_ready())
        tud_hid_n_report(instance, 0x00, &packet_tx, SIZE_PACKET_TX_T);

    // clang-format off
    // #if (CFG_TUD_CDC == 1)
    //     if (tud_cdc_ready()) {
    //         tud_cdc_write(&packet_tx, SIZE_PACKET_TX_T);
    //         tud_cdc_write_flush();
    //     }
    // #endif

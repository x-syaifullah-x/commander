#define _STRINGIFY(x) #x
#define STRINGIFY(x) _STRINGIFY(x)

#define USB_ID_VENDOR 0x1902
#define USB_ID_PRODUCT 0x1992
#define USB_MANUFACTIRER "Raspberry PI Pico"
#define USB_PRODUCT "RP2040"
#define USB_SERIAL_NUMBER "E0C9125B0D9B"

#define TB6612FNG_PIN_STBY PIN_04

#define TB6612FNG_PIN_IN_1_A PIN_07
#define TB6612FNG_PIN_IN_2_A PIN_06
#define TB6612FNG_PIN_PWM_A PIN_05

#define TB6612FNG_PIN_IN_1_B PIN_09
#define TB6612FNG_PIN_IN_2_B PIN_10
#define TB6612FNG_PIN_PWM_B PIN_11

#include "bsp/board.h"
#include "handlers/led_default.h"
#include "hardware/adc.h"
#include "hardware/gpio.h"
#include "include/pin.h"
#include "pico/binary_info.h"
#include "pico/stdlib.h"

// clang-format off
bi_decl(bi_program_name("raspberry_pi_pico"))
bi_decl(bi_program_version_string("1.0"))
bi_decl(bi_program_url("https://github.com/x-syaifullah-x/raspberry_pi_pico_commander"))
bi_decl(bi_program_description(
    "Device Controller\n"
    "\n"
    "ID Vendor      : " STRINGIFY(USB_ID_VENDOR)
    "\n"
    "ID Product     : " STRINGIFY(USB_ID_PRODUCT)
    "\n"
    "Manufacturer   : " USB_MANUFACTIRER
    "\n"
    "Product        : " USB_PRODUCT
    "\n\n"
    "Power Pins:\n"
    "VBUS    (5V USB)      -> PIN_40\n"
    "VSYS    (2-5V input)  -> PIN_39\n"
    "3V3OUT  (3.3V output) -> PIN_36\n"
    "GND     (Ground)      -> PIN_03, PIN_08, PIN_13, PIN_18, PIN_23, PIN_28, PIN_33, PIN_38"
    "\n"
))

bi_decl(bi_1pin_with_name(TB6612FNG_PIN_STBY, "PIN_04 <-> STBY (TB6612FNG)"))
bi_decl(bi_1pin_with_name(TB6612FNG_PIN_PWM_A, "PIN_05 <-> PWMA (TB6612FNG)"))
bi_decl(bi_1pin_with_name(TB6612FNG_PIN_IN_2_A, "PIN_06 <-> AIN2 (TB6612FNG) --> AO2 (TB6612FNG) <-> MOTOR -"))
bi_decl(bi_1pin_with_name(TB6612FNG_PIN_IN_1_A, "PIN_07 <-> AIN1 (TB6612FNG) --> AO1 (TB6612FNG) <-> MOTOR +\nPIN_08 <-> GND  (TB6612FNG)"))
bi_decl(bi_1pin_with_name(TB6612FNG_PIN_IN_1_B, "PIN_09 <-> BIN1 (TB6612FNG) --> BO1 (TB6612FNG) <-> MOTOR +"))
bi_decl(bi_1pin_with_name(TB6612FNG_PIN_IN_2_B, "PIN_10 <-> BIN2 (TB6612FNG) --> BO2 (TB6612FNG) <-> MOTOR -"))
bi_decl(bi_1pin_with_name(TB6612FNG_PIN_PWM_B, "PIN_11 <-> PWMB (TB6612FNG)\n---------- VM   (TB6612FNG) <-> POWER +\nPIN_36 <-> VCC  (TB6612FNG)\n---------- GND  (TB6612FNG) <-> POWER -"))
    
static const char *string_desc_arr[] = {
        (const char[]){0x09, 0x04}, // Language ID(English US)
        USB_MANUFACTIRER,
        USB_PRODUCT,
        USB_SERIAL_NUMBER,
        "CDC Control", // CDC Interface
        "CDC Data", // CDC Interface
        "HID Control", // HID Interface
        "Bulk Data", // Vendor Interface
};  // clang-format on

uint16_t const *tud_descriptor_string_cb(uint8_t index, uint16_t langid) {
    (void)langid;

    static uint16_t desc_str[32];
    uint8_t chr_count;

    if (index == 0) {
        memcpy(&desc_str[1], string_desc_arr[0], 2);
        chr_count = 1;
    } else {
        if (index >= sizeof(string_desc_arr) / sizeof(string_desc_arr[0]))
            return NULL;

        const char *str = string_desc_arr[index];
        chr_count = strlen(str);

        if (chr_count > 31) chr_count = 31;

        for (uint8_t i = 0; i < chr_count; i++) {
            desc_str[1 + i] = str[i];  // ASCII → UTF-16LE
        }
    }

    // Header
    desc_str[0] = (TUSB_DESC_STRING << 8) | (2 * chr_count + 2);
    return desc_str;
}

enum {
#if CFG_TUD_CDC
    ITF_NUM_CDC,
    ITF_NUM_CDC_DATA,
#endif
#if CFG_TUD_HID
    ITF_NUM_HID,
#endif
#if CFG_TUD_VENDOR
    ITF_NUM_VENDOR,
#endif
    ITF_NUM_TOTAL
};

#define USB_CDC_DESCRIPTOR_(_itfnum, _stridx, _stridx_data, _ep_notif, _ep_notif_size, _epout, _epin, _epsize_out, _epsize_in)                                                                                               \
    /* Interface Associate */                                                                                                                                                                                                \
    8, TUSB_DESC_INTERFACE_ASSOCIATION, _itfnum, 2, TUSB_CLASS_CDC, CDC_COMM_SUBCLASS_ABSTRACT_CONTROL_MODEL, CDC_COMM_PROTOCOL_NONE, 0,                                                         /* CDC Control Interface */ \
        9, TUSB_DESC_INTERFACE, _itfnum, 0, 1, TUSB_CLASS_CDC, CDC_COMM_SUBCLASS_ABSTRACT_CONTROL_MODEL, CDC_COMM_PROTOCOL_NONE, _stridx,                                                        /* CDC Header */            \
        5, TUSB_DESC_CS_INTERFACE, CDC_FUNC_DESC_HEADER, U16_TO_U8S_LE(0x0120),                                                                                                                  /* CDC Call */              \
        5, TUSB_DESC_CS_INTERFACE, CDC_FUNC_DESC_CALL_MANAGEMENT, 0, (uint8_t)((_itfnum) + 1),                                                                                                   /* CDC ACM */               \
        4, TUSB_DESC_CS_INTERFACE, CDC_FUNC_DESC_ABSTRACT_CONTROL_MANAGEMENT, 6,                                                                                                                 /* CDC Union */             \
        5, TUSB_DESC_CS_INTERFACE, CDC_FUNC_DESC_UNION, _itfnum, (uint8_t)((_itfnum) + 1),                                                                                                       /* Endpoint Notification */ \
        7, TUSB_DESC_ENDPOINT, _ep_notif, TUSB_XFER_INTERRUPT, U16_TO_U8S_LE(_ep_notif_size), 1,                                                                                                 /* CDC Data Interface */    \
        9, TUSB_DESC_INTERFACE, (uint8_t)((_itfnum) + 1), 0, 2, TUSB_CLASS_CDC_DATA, 0, 0, _stridx_data, /*                                                                            ↑ fix! */ /* Endpoint Out */          \
        7, TUSB_DESC_ENDPOINT, _epout, TUSB_XFER_BULK, U16_TO_U8S_LE(_epsize_out), 0,                                                                                                            /* Endpoint In */           \
        7, TUSB_DESC_ENDPOINT, _epin, TUSB_XFER_BULK, U16_TO_U8S_LE(_epsize_in), 0

#define TUD_VENDOR_DESCRIPTOR_(_itfnum, _stridx, _epout, _epin, _epsize_out, _epsize_in)                       \
    /* Interface */                                                                                            \
    9, TUSB_DESC_INTERFACE, _itfnum, 0, 2, TUSB_CLASS_VENDOR_SPECIFIC, 0x00, 0x00, _stridx, /* Endpoint Out */ \
        7, TUSB_DESC_ENDPOINT, _epout, TUSB_XFER_BULK, U16_TO_U8S_LE(_epsize_out), 0,       /* Endpoint In */  \
        7, TUSB_DESC_ENDPOINT, _epin, TUSB_XFER_BULK, U16_TO_U8S_LE(_epsize_in), 0

#define ROUND_UP_8(x) (((x) + 7) & ~7)

// clang-format off
#define ROUND_UP_POW2(x) ((x) <= 8  ? 8  : (x) <= 16 ? 16 : (x) <= 32 ? 32 : 64)
// clang-format on

#define USB_CDC_DESCRIPTOR USB_CDC_DESCRIPTOR_(ITF_NUM_CDC, 4, 5, 0x81, 8, 0x02, 0x82, ROUND_UP_POW2(SIZE_PACKET_RX_T), ROUND_UP_POW2(SIZE_PACKET_TX_T))
#define USB_HID_DESCRIPTOR TUD_HID_INOUT_DESCRIPTOR(ITF_NUM_HID, 6, HID_ITF_PROTOCOL_NONE, sizeof(hid_descriptor_report), 0x83, 0x03, 64, 1)
#define USB_VENDOR_DESCRIPTOR TUD_VENDOR_DESCRIPTOR_(ITF_NUM_VENDOR, 7, 0x04, 0x84, ROUND_UP_POW2(SIZE_PACKET_RX_T), ROUND_UP_POW2(SIZE_PACKET_TX_T))

// clang-format off
#if CFG_TUD_CDC && CFG_TUD_HID && CFG_TUD_VENDOR
#include "usb_cdc.h" 
#include "usb_hid.h"
#include "usb_vendor.h"

bi_decl(bi_program_feature("CDC,HID,VENDOR"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_CDC_DESC_LEN + TUD_HID_INOUT_DESC_LEN + TUD_VENDOR_DESC_LEN), 0, 100),
    USB_CDC_DESCRIPTOR,
    USB_HID_DESCRIPTOR,
    USB_VENDOR_DESCRIPTOR,
};
#elif CFG_TUD_CDC && CFG_TUD_HID
#include "usb_cdc.h"
#include "usb_hid.h"

bi_decl(bi_program_feature("CDC,HID"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_CDC_DESC_LEN + TUD_HID_INOUT_DESC_LEN), 0, 100),
    USB_CDC_DESCRIPTOR,
    USB_HID_DESCRIPTOR,
};
#elif CFG_TUD_CDC && CFG_TUD_VENDOR
#include "usb_cdc.h"
#include "usb_vendor.h"

bi_decl(bi_program_feature("CDC,VENDOR"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_CDC_DESC_LEN + TUD_VENDOR_DESC_LEN), 0, 100),
    USB_CDC_DESCRIPTOR,
    USB_VENDOR_DESCRIPTOR,
};
#elif CFG_TUD_HID && CFG_TUD_VENDOR
#include "usb_hid.h"
#include "usb_vendor.h"

bi_decl(bi_program_feature("HID,VENDOR"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_HID_INOUT_DESC_LEN + TUD_VENDOR_DESC_LEN), 0, 100),
    USB_HID_DESCRIPTOR,
    USB_VENDOR_DESCRIPTOR,
};
#elif CFG_TUD_CDC
#include "usb_cdc.h"

bi_decl(bi_program_feature("CDC"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_CDC_DESC_LEN), 0, 100),
    USB_CDC_DESCRIPTOR,
};
#elif CFG_TUD_HID
#include "usb_hid.h"

bi_decl(bi_program_feature("HID"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_HID_INOUT_DESC_LEN), 0, 100),
    USB_HID_DESCRIPTOR,
};
#elif CFG_TUD_VENDOR
#include "usb_vendor.h"

bi_decl(bi_program_feature("VENDOR"))

static uint8_t const descriptor_configuration[] = {
    TUD_CONFIG_DESCRIPTOR(1, ITF_NUM_TOTAL, 0, (TUD_CONFIG_DESC_LEN + TUD_VENDOR_DESC_LEN), 0, 100),
    USB_VENDOR_DESCRIPTOR,
};
#else
#error "No active USB class defined"
#endif
// clang-format on

uint8_t const *tud_descriptor_configuration_cb(uint8_t index) {
    (void)index;

    return descriptor_configuration;
}

tusb_desc_device_t const desc_device = {
    .bLength = sizeof(tusb_desc_device_t),
    .bDescriptorType = TUSB_DESC_DEVICE,
    .bcdUSB = 0x0200,

    .bDeviceClass = 0x00,
    .bDeviceSubClass = 0x00,
    .bDeviceProtocol = 0x00,

    .bMaxPacketSize0 = CFG_TUD_ENDPOINT0_SIZE,

    .idVendor = USB_ID_VENDOR,
    .idProduct = USB_ID_PRODUCT,
    .bcdDevice = 0x0100,

    .iManufacturer = 0x01,
    .iProduct = 0x02,
    .iSerialNumber = 0x03,

    .bNumConfigurations = 0x01,
};

uint8_t const *tud_descriptor_device_cb(void) {
    return (uint8_t const *)&desc_device;
}

int main(void) {
    // clang-format off
    #if defined(LIB_PICO_STDIO_USB)
        stdio_usb_init();
    #endif
    // clang-format on

    // LED
    gpio_init(PICO_DEFAULT_LED_PIN);
    gpio_set_dir(PICO_DEFAULT_LED_PIN, GPIO_OUT);
    led_blink_stat = add_repeating_timer_ms(1000, led_blink, NULL, &timer);

    // TEMP
    adc_init();
    adc_set_temp_sensor_enabled(true);

    tusb_init();

    while (true) {
        tud_task();
    }
}